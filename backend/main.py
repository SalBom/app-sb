...
# ---------------------------------------------------------------
# GESTIÓN DE USUARIOS (HÍBRIDO: ODOO + POSTGRESQL)
# ---------------------------------------------------------------

# main.py - Reemplazar la función get_users existente
...
def get_users():
    ...
    try:
        ...
        domain = [('active', '=', True)]
        fields = ['id', 'name', 'login', 'image_128', 'partner_id']
        
        users_odoo = client.env['res.users'].search_read(
            domain, fields, order='name asc', limit=300
        )

        # 2. Obtener los CUITs (vat) de los partners asociados
        partner_ids = [u['partner_id'][0] for u in users_odoo if u.get('partner_id')]
        vat_map = {}
        
        if partner_ids:
            partners_data = client.env['res.partner'].search_read(
                [('id', 'in', partner_ids)], ['id', 'vat']
            )
            for p in partners_data:
                vat_map[p['id']] = p.get('vat') or ""

        # 3. Traer roles de PostgreSQL
        roles_map = {}
        if DATABASE_URL:
            pg_conn = get_pg_connection()
            if pg_conn:
                try:
                    cur = pg_conn.cursor()
                    cur.execute("SELECT user_id, role_name FROM app_user_roles")
                    rows = cur.fetchall()
                    for r in rows:
                        roles_map[r[0]] = r[1]
                    cur.close()
                except Exception as e:
                    log.error(f"Error roles PG: {e}")

        # 4. Armar respuesta
        users_formatted = []
        for u in users_odoo:
            uid = u['id']
            rol_asignado = roles_map.get(uid, 'Sin Asignar')
            
            # Obtener CUIT del mapa
            pid = u.get('partner_id')
            cuit = ""
            if pid and isinstance(pid, (list, tuple)):
                cuit = vat_map.get(pid[0], "")

            users_formatted.append({
                'id': uid,
                'name': u['name'],
                'login': u['login'], 
                'image_128': u.get('image_128') or None,
                'role': rol_asignado,
                'cuit': cuit  # <--- DATO CLAVE
            })
