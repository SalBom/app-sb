// src/screens/DashboardAdministrador.tsx
...
// --- AJUSTE DE ALTURAS (Más compacto) ---
...
const DashboardAdministrador: React.FC = () => {
  ...
  const cargarFacturas = async (cuit: string) => {
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        setErrorFacturas(json?.error || 'No se pudieron cargar facturas.');
        return;
      }
    ...
    <ScrollView style={s.screen} contentContainerStyle={s.container} bounces={false}>
      ...
      <View style={s.header}>
        <View style={s.headerTitleRow}>
            <FlechaHeaderSvg width={45} height={28} style={{ marginLeft: -10 }} />
            <Text style={s.title}>ADMIN</Text>
        </View>
        <View style={s.headerControls}>
            <TouchableOpacity 
                style={[s.pickerBtn, { marginRight: 8 }]} 
                onPress={() => setVendorPickerVisible(true)}
            >
                <Text style={s.pickerText} numberOfLines={1} ellipsizeMode='tail'>
                    {selectedVendor ? selectedVendor.name : 'Seleccionar...'}
                </Text>
                <Ionicons name="caret-down" size={10} color="#2B2B2B" style={{ marginLeft: 4 }} />
            </TouchableOpacity>
            <TouchableOpacity 
                style={s.pickerBtn} 
            ...
            >
                <Text style={s.pickerText}>
                    {MONTHS[selectedMonth - 1].substring(0,3).toUpperCase()} {selectedYear}
                </Text>
                <Ionicons name="chevron-down" size={12} color="#2B2B2B" style={{ marginLeft: 4 }} />
            </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={s.topSectionRow}>
        <TouchableOpacity style={s.graficosButton} activeOpacity={0.8}>
          {/* Fondo hexagonal visual */}
        ...
        <View style={s.rightColumn}>
                ...
                <View style={s.kpiPaddingBox}>
                  ...
                  {loadingKpi ? (
                    ...
                    <View style={s.kpiItemsWrapper}>
                      ...
                      <TouchableOpacity style={s.kpiItem} onPress={() => onPressEstado('atendidos')} activeOpacity={0.7}>
          ...
          {/* HISTORIAL hexagonales */}
          <View style={s.historialSection}>
            <TouchableOpacity style={s.historialBtnShadow} activeOpacity={0.8}
              onPress={() => navigation.navigate('Facturas', { cuitOverride: selectedVendor?.cuit })}>
            ...
            <TouchableOpacity style={s.historialBtnShadow} activeOpacity={0.8}
              onPress={() => navigation.navigate('Pedidos', { cuitOverride: selectedVendor?.cuit })}>
              <View style={s.historialBtnInner}>
                <ContenedorHistorialSvg style={StyleSheet.absoluteFill} width="100%" height="100%" preserveAspectRatio="none"/>
                <View style={s.historialBtnContent}>
        ...
        <View style={s.ultimosShadowWrap}>
          ...
          <View style={s.ultimosContent}>
            ...
            {!loadingFacturas && facturas.length === 0 && (
              <View style={s.ultimosEmpty}>
                <Text style={s.ultimosEmptyText}>No hay facturas en este período.</Text>
              </View>
            )}
                ...
                >
                  <View style={s.pedidoTextWrap}>
                    ...
                    <Text style={s.pedidoMeta}>
                      <Text style={s.pedidoMetaLabel}>CLIENTE: </Text>{f.cliente || 'Desconocido'}
                    </Text>
                    <Text style={s.pedidoMeta}>
                      <Text style={s.pedidoMetaLabel}>FECHA: </Text>{formatFecha(f.fecha)}
                    </Text>
                  </View>
                  <View style={s.pedidoArrowWrap}>
                    <ArrowPedidosSvg width={18} height={18} />
                  </View>
                </TouchableOpacity>
              ))}
          </View>
      ...
      {/* MODAL VENDEDOR */}
      <Modal visible={vendorPickerVisible} transparent={true} animationType="fade" onRequestClose={() => setVendorPickerVisible(false)}>
        <Pressable style={s.modalOverlay} onPress={() => setVendorPickerVisible(false)}>
            <View style={s.modalContent}>
                ...
                />
                <TouchableOpacity style={s.modalCloseBtn} onPress={() => setVendorPickerVisible(false)}>
...
const s = StyleSheet.create({
  ...
  /* Top Section hexagonales y espaciados */
  topSectionRow: { 
    flexDirection: 'row', 
  ...
  estadoShadowWrap: {
    marginTop: 4, borderRadius: 8, backgroundColor: '#FFFFFF',
  ...
  ultimosShadowWrap: {
    height: ULTIMOS_CARD_HEIGHT, marginHorizontal: 10,
